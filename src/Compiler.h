...
public:
	...
	*/
	...
	const Token& CurrentToken() const { return m_Parser.tokens.back(); }
	const Token& PreviousToken() const { return *(m_Parser.tokens.end() - 2); }
	const Token& TokenAt(size_t i) const { return *(m_Parser.tokens.rbegin() + i); }
...
private:
	...
	*/
	...
	void addToken(const Token& token) { m_Parser.tokens.push_back(token); }
